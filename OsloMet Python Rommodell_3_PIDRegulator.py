...
P_sol_faktor=0.5                          #Gir mulighet for å gange opp solstrålingen med en fakor
...
def PID(Kp, Ti, Td, T_ute, P_sol):
    ...
    for i in range(1,24):
        ...
        nT_masse=[0]*n
        nT_rom=[0]*n
        nT_masse[0]=T_masse[i-1]
        nT_rom[0]=T_rom[i-1]
        
        for j in range (1,n):
            P[2]=nT_masse[j-1]
            deltaT[j]=T_onsket-nT_rom[j-1]
            u=Kp*deltaT[j] + I[-1] + (Kp*Ts/Ti)*deltaT[j] + (Kp*Td/Ts)*(deltaT[j]-deltaT[j-1])
            I.append(I[-1] + (Kp*Ts/Ti)*deltaT[j])
            if(u>0):
                if(u>P_varme_max):
                    u=P_varme_max
                P[6]=u
        ...
        P_varme[i]=P[6]
        P_kjol[i]=P[7]
    return T_rom, T_vegg, T_masse, P_varme, P_kjol

print(PI_Kp, PI_Ti, PI_Td, T_ute, P_sol)
T_rom, T_vegg, T_masse, P_varme, P_kjol = PID(PI_Kp, PI_Ti, PI_Td, T_ute, P_sol)
rommodell_plot_temp(T_ute, T_rom, T_vegg, T_masse)
rommodell_plot_effekt(P_sol, P_varme, P_kjol)
